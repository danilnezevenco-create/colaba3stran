...
public class btr82Model extends VehicleModel<btr82Entity> {
    private static final ResourceLocation CACHED_RESOURCE_0 = new ResourceLocation("squadmc", "geo/btr82.geo.json");
    ...
    @Override
    public ResourceLocation getTextureResource(btr82Entity animatable) {

        return CACHED_RESOURCE_1;
    }

    @Override
    public ResourceLocation getAnimationResource(btr82Entity animatable) {
    ...
    private float getKeyframeValue(float t, int component) {
        int wrapRange = 150;
        float normalized = t / (float)wrapRange * 75.0F;
        int idx1 = Mth.clamp((int)normalized, 0, 75);
        int idx2 = Mth.clamp(idx1 + 1, 0, 75);
    ...
    public float getBoneRotX(float t) { return this.getKeyframeValue(t, 0); }
    public float getBoneMoveY(float t) { return this.getKeyframeValue(t, 1) - 19.5F; }
    public float getBoneMoveZ(float t) { return this.getKeyframeValue(t, 2) - 59.0F; }
    ...
    @Override
    public VehicleModel.@Nullable TransformContext<btr82Entity> collectTransform(String boneName) {
        VehicleModel.TransformContext<btr82Entity> var10000;
        switch (boneName) {
            case "WheelTurnL1":
            case "WheelTurnL2":
            case "WheelTurnR1":
