...
@OnlyIn(Dist.CLIENT)
public class sprutSoundManager {
    private static final WeakHashMap<sprutEntity, sprutEngineSoundInstance> activeSounds = new WeakHashMap<>();

    public static void updateSounds(sprutEntity vehicle) {
        if (vehicle.isRemoved() || !vehicle.isAlive() || !vehicle.isEngineStarted()) {
            sprutEngineSoundInstance sound = activeSounds.remove(vehicle);
            if (sound != null) {
                sound.startFadeOut();
            }
            return;
        ...
        sprutEngineSoundInstance currentSound = activeSounds.get(vehicle);
        ...
        if (currentSound == null || currentSound.isStopped() ||
            ...
            sprutEngineSoundInstance newSound = new sprutEngineSoundInstance(vehicle, isFirstPerson, isIdle);
