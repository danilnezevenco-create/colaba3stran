...
public class m1a2EngineSoundInstance extends AbstractTickableSoundInstance {
    private final m1a2Entity vehicle;
    private final boolean isFirstPerson;
    private final boolean isIdle;
    private boolean fadingOut = false;
    private float targetVolume = 1.0F;

    public m1a2EngineSoundInstance(m1a2Entity vehicle, boolean isFirstPerson, boolean isIdle) {
        super(isIdle ?
                        (isFirstPerson ? ModSounds.M1A2_ENGINE_IDLE_1P.get() : ModSounds.M1A2_ENGINE_IDLE_3P.get()) :
                        (isFirstPerson ? ModSounds.M1A2_RUNNING_1P.get() : ModSounds.M1A2_RUNNING_3P.get()),
    ...
    public void tick() {
        ...
        if (this.isIdle) {
            this.pitch = 1.0F + (float)Math.sin(this.vehicle.tickCount * 0.05D) * 0.01F;
        } else {
            float speedFactor = (float)(speed * 1.6D);
            this.pitch = Mth.clamp(0.9F + speedFactor, 0.9F, 1.35F);
        }
    }
