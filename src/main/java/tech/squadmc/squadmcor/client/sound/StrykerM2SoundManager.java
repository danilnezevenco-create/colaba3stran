...
@OnlyIn(Dist.CLIENT)
public class StrykerM2SoundManager {
    private static final WeakHashMap<StrykerM2Entity, StrykerM2EngineSoundInstance> activeSounds = new WeakHashMap<>();

    public static void updateSounds(StrykerM2Entity vehicle) {
        if (vehicle.isRemoved() || !vehicle.isAlive() || !vehicle.isEngineStarted()) {
            StrykerM2EngineSoundInstance sound = activeSounds.remove(vehicle);
            if (sound != null) {
                sound.startFadeOut();
            }
            return;
        ...
        StrykerM2EngineSoundInstance currentSound = activeSounds.get(vehicle);
        ...
        if (currentSound == null || currentSound.isStopped() ||
            ...
            StrykerM2EngineSoundInstance newSound = new StrykerM2EngineSoundInstance(vehicle, isFirstPerson, isIdle);
