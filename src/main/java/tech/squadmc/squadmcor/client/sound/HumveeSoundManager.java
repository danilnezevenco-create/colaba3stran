...
@OnlyIn(Dist.CLIENT)
public class HumveeSoundManager {
    private static final WeakHashMap<HumveeEntity, HumveeEngineSoundInstance> activeSounds = new WeakHashMap<>();

    public static void updateSounds(HumveeEntity vehicle) {
        if (vehicle.isRemoved() || !vehicle.isAlive() || !vehicle.isEngineStarted()) {
            HumveeEngineSoundInstance sound = activeSounds.remove(vehicle);
            if (sound != null) {
                sound.startFadeOut();
            }
            return;
        ...
        HumveeEngineSoundInstance currentSound = activeSounds.get(vehicle);
        ...
        if (currentSound == null || currentSound.isStopped() ||
            ...
            HumveeEngineSoundInstance newSound = new HumveeEngineSoundInstance(vehicle, isFirstPerson, isIdle);
