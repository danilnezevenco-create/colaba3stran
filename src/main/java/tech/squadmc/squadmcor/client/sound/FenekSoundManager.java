...
@OnlyIn(Dist.CLIENT)
public class FenekSoundManager {
    private static final WeakHashMap<FenekEntity, FenekEngineSoundInstance> activeSounds = new WeakHashMap<>();

    public static void updateSounds(FenekEntity vehicle) {
        if (vehicle.isRemoved() || !vehicle.isAlive() || !vehicle.isEngineStarted()) {
            FenekEngineSoundInstance sound = activeSounds.remove(vehicle);
            if (sound != null) {
                sound.startFadeOut();
            }
            return;
        ...
        FenekEngineSoundInstance currentSound = activeSounds.get(vehicle);
        ...
        if (currentSound == null || currentSound.isStopped() ||
            ...
            FenekEngineSoundInstance newSound = new FenekEngineSoundInstance(vehicle, isFirstPerson, isIdle);
