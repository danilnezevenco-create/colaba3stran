...
@OnlyIn(Dist.CLIENT)
public class leopardSoundManager {
    private static final WeakHashMap<leopardEntity, leopardEngineSoundInstance> activeSounds = new WeakHashMap<>();

    public static void updateSounds(leopardEntity vehicle) {
        if (vehicle.isRemoved() || !vehicle.isAlive() || !vehicle.isEngineStarted()) {
            leopardEngineSoundInstance sound = activeSounds.remove(vehicle);
            if (sound != null) {
                sound.startFadeOut();
            }
            return;
        ...
        leopardEngineSoundInstance currentSound = activeSounds.get(vehicle);
        ...
        if (currentSound == null || currentSound.isStopped() ||
            ...
            leopardEngineSoundInstance newSound = new leopardEngineSoundInstance(vehicle, isFirstPerson, isIdle);
