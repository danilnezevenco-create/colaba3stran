...
@OnlyIn(Dist.CLIENT)
public class PumaSoundManager {
    private static final WeakHashMap<PumaEntity, PumaEngineSoundInstance> activeSounds = new WeakHashMap<>();

    public static void updateSounds(PumaEntity vehicle) {
        if (vehicle.isRemoved() || !vehicle.isAlive() || !vehicle.isEngineStarted()) {
            PumaEngineSoundInstance sound = activeSounds.remove(vehicle);
            if (sound != null) {
                sound.startFadeOut();
            }
            return;
        ...
        PumaEngineSoundInstance currentSound = activeSounds.get(vehicle);
        ...
        if (currentSound == null || currentSound.isStopped() ||
            ...
            PumaEngineSoundInstance newSound = new PumaEngineSoundInstance(vehicle, isFirstPerson, isIdle);
