...
@OnlyIn(Dist.CLIENT)
public class UralGradSoundManager {
    private static final WeakHashMap<UralGradEntity, UralGradEngineSoundInstance> activeSounds = new WeakHashMap<>();

    public static void updateSounds(UralGradEntity vehicle) {
        if (vehicle.isRemoved() || !vehicle.isAlive() || !vehicle.isEngineStarted()) {
            UralGradEngineSoundInstance sound = activeSounds.remove(vehicle);
            if (sound != null) {
                sound.startFadeOut();
            }
            return;
        ...
        UralGradEngineSoundInstance currentSound = activeSounds.get(vehicle);
        ...
        if (currentSound == null || currentSound.isStopped() ||
            ...
            UralGradEngineSoundInstance newSound = new UralGradEngineSoundInstance(vehicle, isFirstPerson, isIdle);
