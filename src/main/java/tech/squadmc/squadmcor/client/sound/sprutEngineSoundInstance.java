...
public class sprutEngineSoundInstance extends AbstractTickableSoundInstance {
    private final sprutEntity vehicle;
    private final boolean isFirstPerson;
    private final boolean isIdle;
    private boolean fadingOut = false;
    private float targetVolume = 1.0F;

    public sprutEngineSoundInstance(sprutEntity vehicle, boolean isFirstPerson, boolean isIdle) {
        super(isIdle ?
                        (isFirstPerson ? ModSounds.T72AV_ENGINE_IDLE_1P.get() : ModSounds.T72AV_ENGINE_IDLE_3P.get()) :
                        (isFirstPerson ? ModSounds.T72AV_ENGINE_RUNNING_1P.get() : ModSounds.T72AV_ENGINE_RUNNING_3P.get()),
        ...
        // РўРђРќРљР: РЎС‚РѕРёС‚ = 54 (3.375F), Р•РґРµС‚ = 110 (6.875F)
        if (isFirstPerson) {
            this.targetVolume = 1.0F;
        } else {
            this.targetVolume = isIdle ? 3.375F : 6.875F;
        }
    ...
    public void tick() {
        ...
        if (this.isIdle) {
            this.pitch = 1.0F + (float)Math.sin(this.vehicle.tickCount * 0.08D) * 0.02F;
        } else {
            float speedFactor = (float)(speed * 1.8D);
