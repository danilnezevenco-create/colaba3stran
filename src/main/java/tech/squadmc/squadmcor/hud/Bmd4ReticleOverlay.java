...
public class Bmd4ReticleOverlay {
    ...
    private static final ResourceLocation BMD4_CANNON_SCOPE = new ResourceLocation(squadmc.MODID, "textures/overlay/btr82_scope.png");
    ...
    public static void onRenderGuiPre(RenderGuiEvent.Pre event) {
        ...
        if (!(vehicle instanceof bmd4Entity bmd)) {
            return;
        }

        int seatIndex = bmd.getSeatIndex(player);
        if (seatIndex < 0) {
            return;
        }
        ...
        if (isFirstPerson || isZooming) {
            ...
            if (seatIndex == 0) {
                ...
                drawDriverHud(guiGraphics, mc, bmd, screenWidth, screenHeight);
            }
            else if (seatIndex == 1) {
                guiGraphics.blit(GUNNER_TEXTURE, 0, 0, 0, 0, screenWidth, screenHeight, screenWidth, screenHeight);

                int selectedWeapon = getSelectedWeaponIndex(bmd, seatIndex);

                ResourceLocation activeScope;
                activeScope = BMD4_CANNON_SCOPE;
    ...
    private static void drawDriverHud(GuiGraphics guiGraphics, Minecraft mc, bmd4Entity bmd, int width, int height) {
        double speed = bmd.getDeltaMovement().horizontalDistance() * 72.0D;
        String speedText = String.format("SPEED: %.0f km/h", speed);

        int healthPercent = (int) ((bmd.getHealth() / bmd.getMaxHealth()) * 100);
    ...
    private static int getSelectedWeaponIndex(bmd4Entity bmd, int seatIndex) {
        if (searchedReflection) {
            try {
                if (cachedMethod != null) {
                    return (int) cachedMethod.invoke(bmd, seatIndex);
                }
                if (cachedField != null) {
                    int[] arr = (int[]) cachedField.get(bmd);
            ...
            for (Method m : clazz.getMethods()) {
                ...
                if (name.contains("weapon") && (name.contains("select") || name.contains("index") || name.contains("current"))) {
                    if (m.getParameterCount() == 1 && m.getParameterTypes()[0] == int.class && m.getReturnType() == int.class) {
                        ...
                        cachedMethod = m;
                        return (int) m.invoke(bmd, seatIndex);
                    }
                }
            }
        } catch (Exception e) {}
