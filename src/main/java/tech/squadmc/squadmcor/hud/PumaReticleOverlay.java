...
public class PumaReticleOverlay {
    ...
    private static final ResourceLocation PUMA_SCOPE = new ResourceLocation(squadmc.MODID, "textures/overlay/puma_scope.png");
    ...
    public static void onRenderGuiPre(RenderGuiEvent.Pre event) {
        ...
        if (!(vehicle instanceof PumaEntity puma)) {
            return;
        }

        int seatIndex = puma.getSeatIndex(player);
        if (seatIndex < 0) {
            return;
        }
        ...
        if (isFirstPerson || isZooming) {
            ...
            if (seatIndex == 0) {
                ...
                drawDriverHud(guiGraphics, mc, puma, screenWidth, screenHeight);
            } else if (seatIndex == 1) {
                ...
                guiGraphics.blit(PUMA_SCOPE, 0, 0, 0, 0, screenWidth, screenHeight, screenWidth, screenHeight);
    ...
    private static void drawDriverHud(GuiGraphics guiGraphics, Minecraft mc, PumaEntity puma, int width, int height) {
        double speed = puma.getDeltaMovement().horizontalDistance() * 72.0D;
        String speedText = String.format("SPD: %.0f km/h", speed);

        int healthPercent = (int) ((puma.getHealth() / puma.getMaxHealth()) * 100);
