...
public class T72AVReticleOverlay {
    ...
    private static final ResourceLocation T72_SCOPE = new ResourceLocation("squadmc", "textures/overlay/t72_scope.png");
    ...
    public static void onRenderGuiPre(RenderGuiEvent.Pre event) {
        ...
        if (!(vehicle instanceof T72AVEntity t72)) {
            return;
        }

        int seatIndex = t72.getSeatIndex(player);
        if (seatIndex < 0) {
            return;
        }
        ...
        if (isFirstPerson || isZooming) {
            ...
            if (seatIndex == 0) {
                ...
                drawDriverHud(guiGraphics, mc, t72, screenWidth, screenHeight);
            } else if (seatIndex == 1) {
                guiGraphics.blit(GUNNER_TEXTURE, 0, 0, 0, 0, screenWidth, screenHeight, screenWidth, screenHeight);
                guiGraphics.blit(T72_SCOPE, 0, 0, 0, 0, screenWidth, screenHeight, screenWidth, screenHeight);
                ...
                drawCommanderHud(guiGraphics, mc, t72, player, screenWidth, screenHeight);
            }
    ...
    private static void drawDriverHud(GuiGraphics guiGraphics, Minecraft mc, T72AVEntity t72, int width, int height) {
        double speed = t72.getDeltaMovement().horizontalDistance() * 72.0D;
        String speedText = String.format("SPEED: %.0f km/h", speed);

        int healthPercent = (int) ((t72.getHealth() / t72.getMaxHealth()) * 100);
    ...
    private static void drawCommanderHud(GuiGraphics guiGraphics, Minecraft mc, T72AVEntity t72, Player player, int width, int height) {
        int color = 0x66FF00;
