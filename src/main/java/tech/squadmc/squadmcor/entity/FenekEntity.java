...
public class FenekEntity extends SquadBaseVehicleEntity {
    public static final EntityDataAccessor<Float> STEERING_ANGLE =
            SynchedEntityData.defineId(FenekEntity.class, EntityDataSerializers.FLOAT);

    public float prevSteeringAngle = 0.0F;
    public float wheelRotation = 0.0F;
    public float prevWheelRotation = 0.0F;

    public FenekEntity(EntityType<? extends FenekEntity> type, Level world) {
        super(type, world);
    }
    ...
    public void tick() {
        ...
        if (isMoving && Math.abs(currentAngle) > 1.0F) {
            float turnAmount = currentAngle * 0.010F * (float) speed;
        ...
        if (this.level().isClientSide) {
            net.minecraftforge.fml.DistExecutor.unsafeRunWhenOn(net.minecraftforge.api.distmarker.Dist.CLIENT, () -> () ->
                    tech.squadmc.squadmcor.client.ClientAccess.handleFenekClientTick(this)
            );
        }
    }
