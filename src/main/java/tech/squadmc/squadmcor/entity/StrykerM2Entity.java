...
public class StrykerM2Entity extends SquadBaseVehicleEntity {

    public static final EntityDataAccessor<Float> STEERING_ANGLE =
            SynchedEntityData.defineId(StrykerM2Entity.class, EntityDataSerializers.FLOAT);

    public float prevSteeringAngle = 0.0F;
    public float wheelRotation = 0.0F;
    public float prevWheelRotation = 0.0F;

    public StrykerM2Entity(EntityType<? extends StrykerM2Entity> type, Level world) {
        super(type, world);
    }
    ...
    public void tick() {
        ...
        if (this.level().isClientSide) {
            net.minecraftforge.fml.DistExecutor.unsafeRunWhenOn(net.minecraftforge.api.distmarker.Dist.CLIENT, () -> () ->
                    tech.squadmc.squadmcor.client.ClientAccess.handleStrykerM2ClientTick(this)
            );
        }
    }
