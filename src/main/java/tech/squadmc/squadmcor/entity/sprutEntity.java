...
public class sprutEntity extends SquadBaseVehicleEntity {

    public sprutEntity(EntityType<? extends sprutEntity> type, Level world) {
        super(type, world);
    }

    @Override
    public DamageModifier getDamageModifier() {
        return super.getDamageModifier().custom((source, damage) -> this.getSourceAngle(source, 0.7F) * damage);
    }
    ...
    public void tick() {
        ...
        if (this.getShootAnimationTimer(0, 0) > 0) {
            ...
            Vec3 rear = this.position().subtract(look.scale(3.0));

            if (this.level().isClientSide) {
                Vec3 right = new Vec3(-look.z, 0.0, look.x).normalize();

                for (int i = 0; i < 2; i++) {
                    double sideOffset = (this.random.nextDouble() - 0.5) * 4.0;
                    double upOffset = this.random.nextDouble() * 1.2;
                    double backOffset = (this.random.nextDouble() - 0.5) * 0.8;

                    Vec3 spawnPos = rear
                            .add(right.scale(sideOffset))
                            .add(0.0, upOffset + 0.5, 0.0)
                            .add(look.scale(backOffset));

                    double sideSpeed = (this.random.nextDouble() - 0.5) * 0.1;
                    double upSpeed = 0.01 + this.random.nextDouble() * 0.015;
        ...
        if (this.level().isClientSide) {
            net.minecraftforge.fml.DistExecutor.unsafeRunWhenOn(net.minecraftforge.api.distmarker.Dist.CLIENT, () -> () ->
                    tech.squadmc.squadmcor.client.ClientAccess.handleSprutClientTick(this)
            );
        }
    }
    ...
    public void spawnAdditionalGrenades(TDADummyProjectile original) {
        ...
        float[] angles = new float[]{-30.0F, -10.0F, 10.0F, 30.0F};
        float speed = 0.55F;
